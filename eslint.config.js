...
const sdpFpPlugin = require('./tools/eslint-plugin-sdp-fp/index.js');
...
module.exports = [
  {
    ignores: [
      ...
      '**/*.bs.js',
      '**/*.ne.js',
      'packages/sdp-project/src/internal/builtInPatches.js',
    ],
  },
  ...compat.extends(
    'eslint:recommended',
  ...
  ),
  {
    ...
    settings: {
      ...
      'import/ignore': ['_Js'],
      // eslint-plugin-react's "detect" looks for `react` starting from
      // cwd, which is the monorepo root here -- react is only ever a
      // nested workspace package's dependency, never root's, so
      // "detect" can't find it and silently assumes the latest version.
      react: { version: '16.2' },
    },
    ...
    rules: {
      ...
      'no-unused-vars': ['error', {
          argsIgnorePattern: '^_',
          // ESLint 9 changed `no-unused-vars`' own default for this from
          // 'none' to 'all' -- restoring the old default rather than
          // touching every `catch (e) { /* ignored */ }` across the repo,
          // matching the rest of this file's stance on newly-tightened
          // defaults from this dependency bump.
          caughtErrors: 'none',
      }],
