 ...
 *
 * Ported in-repo from eslint-plugin-xod-fp@0.1.1 (last published for
 * eslint@~3.9.1, unmaintained since). The rule logic below is
 * unchanged -- it never touched a deprecated ESLint API (only
 * `context.options` and `context.report`, both stable across every
 * ESLint major since) -- only the packaging changed: this is a real
 * local plugin (see ../index.js) instead of an npm dependency, and
 * the `meta` object gained the `type` field modern ESLint expects.
 * The same rules object is flat-config-ready as-is; see
 * ../index.js for how to register it once this repo's own ESLint
 * version is bumped past classic config.
 */
...
const shouldIgnore = R.curry((opts, node) =>
  R.anyPass([
...
// :: Options -> Node -> Boolean
const isNestingExpression = R.curry((opts, node) =>
  R.both(
...
// :: Options -> Options
const normalizeOptions = R.compose(R.merge(defaultOptions), objectizeOptions);
...
module.exports = {
  ...
  create(context) {
    ...
    return R.converge(R.unapply(R.mergeAll), [
