...
const saveVirtualFile = R.curry((rootDir, virtualFile) => {
  ...
  const writeFn =
    typeof virtualFile.content === 'string' ? writeFile : writeJSON;
  // Write
...
export const saveProjectEntirely = R.curry((dirPath, project) =>
...
export const saveLibraryEntirely = R.curry((owner, project, workspacePath) =>
...
const savePatchChanges = def(
  ...
  (projectDir, patchChanges) => {
    ...
    const deletePatches = R.compose(
      ...
      R.filter(R.compose(R.equals(CHANGE_TYPES.DELETED), R.prop('changeType')))
    );
    ...
    const addModifyPatches = R.compose(
      ...
      R.unnest,
      R.map(R.compose(arrangePatchByFiles, R.prop('data'))),
      R.reject(R.compose(R.equals(CHANGE_TYPES.DELETED), R.prop('changeType')))
    );
...
export const saveProject = def(
  ...
  (projectPath, changes, project) => {
    ...
    return R.compose(
      ...
      R.filter(R.compose(XP.isPathLocal, R.prop('path')))
    )(changes);
  }
);
...
const convertLibPatchToLocalPatch = (patch) => {
  ...
  const newNodes = R.compose(
    ...
    R.filter(R.compose(R.test(libNameRegExp), XP.getNodeType)),
    XP.listNodes
  )(patch);

  return R.compose(XP.upsertNodes(newNodes), XP.setPatchPath(newName))(patch);
};
      ...
      R.map((libName) => {
        ...
        return R.compose(
          ...
          R.map(
            R.when(
              R.has('data'),
              R.over(R.lensProp('data'), convertLibPatchToLocalPatch)
            )
          ),
          R.filter(
            R.compose(R.equals(libName), XP.getLibraryName, R.prop('path'))
          )
        )(changes);
      }),
      R.uniq,
      R.map(R.compose(XP.getLibraryName, R.prop('path'))),
      R.reject(R.compose(XP.isPathLocal, R.prop('path')))
    )(changes)
);
