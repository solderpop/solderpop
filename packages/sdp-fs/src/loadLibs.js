...
// :: string[] -> string[]
const extractLibNamesFromFilenames = libsDir =>
  R.compose(
    R.uniq,
    R.map(
      R.compose(R.join('/'), R.take(2), R.split(path.sep), filename =>
        path.relative(libsDir, filename)
      )
    )
  );
...
// :: Path -> Promise [LibName]
export const scanWorkspaceForLibNames = wsPath =>
  scanForLibNames(resolveLibPath(wsPath));

// :: [Path] -> Map PatchPath Patch
export const loadLibs = libDirs =>
  R.composeP(
    R.mergeAll,
    R.unnest,
    libPairs =>
      Promise.all(
        R.map(([libDir, libs]) => loadLibrary(libs, libDir), libPairs)
      ),
    R.toPairs,
    R.reject(R.isEmpty),
    R.zipObj(libDirs),
