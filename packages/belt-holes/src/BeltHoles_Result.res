...
let flatMap = (res: t<'goodA, 'bad>, fn: 'goodA => t<'goodB, 'bad>): t<'goodB, 'bad> =>
  switch res->map(fn) {
  | Ok(Ok(good)) => Ok(good)
...
let liftM2 = (fn: ('goodA, 'goodB) => t<'goodR, 'bad>, a: t<'goodA, 'bad>, b: t<'goodB, 'bad>): t<
  'goodR,
  'bad,
> =>
  switch (a, b) {
...
let lift2 = (fn: ('goodA, 'goodB) => 'goodR, a: t<'goodA, 'bad>, b: t<'goodB, 'bad>): t<
  'goodR,
  'bad,
...
let lift3 = (
  ...
  b: t<'goodB, 'bad>,
  c: t<'goodC, 'bad>,
): t<'goodR, 'bad> => liftM3((goodA, goodB, goodC) => Ok(fn(goodA, goodB, goodC)), a, b, c)
