...
const getHistoryForPatch = (state, patchPath) =>
  R.pathOr(
...
const getCurrentPatchState = (state, patchPath) =>
  R.compose(getPatchByPathUnsafe(patchPath), getProject)(state);

const HISTORY_LIMIT = 50;

const moveThroughHistory = R.curry(
  (patchPath, takeReplacementFrom, putCurrentTo, state) => {
  ...
  return R.assocPath(
    ...
    R.compose(
      R.over(
        ...
        R.compose(R.take(HISTORY_LIMIT), R.prepend(previousPatchState))
      ),
...
const clearPatchHistory = (patchPath, state) =>
  R.dissocPath(['projectHistory', patchPath], state);
