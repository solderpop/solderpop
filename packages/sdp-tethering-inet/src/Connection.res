...
// file uses Belt's Result, not ReScript's built-in one.
@@warning("-44")
open Belt
...
module Host = {
  ...
  let hostnameRegExp = /^(([a-zA-Z0-9]|[a-zA-Z0-9][a-zA-Z0-9\-]*[a-zA-Z0-9])\.)*([A-Za-z0-9]|[A-Za-z0-9][A-Za-z0-9\-]*[A-Za-z0-9])$/

  let isIp = str => RegExp.test(ipRegExp, str)
  let isHostname = str => RegExp.test(hostnameRegExp, str)

  let validate = str =>
    isIp(str) || isHostname(str) ? Result.Ok(str) : Result.Error(Error.INVALID_HOSTNAME(str))
}
module Port = {
  type t = int
...
module KeepAlive = {
  ...
  let fromString = str =>
    switch Int.fromString(str) {
    | Some(a) if a <= 7200 && a >= 0 => Result.Ok(a)
...
module Link = {
  ...
  let fromString = (str, allowFive) =>
    switch Int.fromString(str) {
    | Some(a) if a >= 0 && a <= (allowFive ? 5 : 4) => Result.Ok(a)
...
let stringify = conn =>
  ...
  | TCP(host, port, keepAlive) =>
    "\"TCP\",\"" ++
    (host ++
    ("\"," ++ (String.make(port) ++ (keepAlive > 0 ? "," ++ String.make(keepAlive) : ""))))
  | SSL(host, port, keepAlive) =>
    "\"TCP\",\"" ++
    (host ++
    ("\"," ++ (String.make(port) ++ (keepAlive > 0 ? "," ++ String.make(keepAlive) : ""))))
  | UDP(host, port) => "\"UDP\",\"" ++ (host ++ ("\"," ++ String.make(port)))
  }
