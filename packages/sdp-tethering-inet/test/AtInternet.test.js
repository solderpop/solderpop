...
const expectPromiseEq = (label, actual, expected) =>
  test(label, () =>
    actual.then(
      ...
      () => {
        throw new Error(`Expected Promise.resolve(${expected}), but got Promise.reject`);
      }
    )
  );
...
describe('AtInternet: basic', () => {
    ...
    return AtInternet.execute(state, 'AT+CIPMUX=1')
      ...
      .then(firstRes =>
        AtInternet.execute(state, 'AT+CIPMUX=0')
          ...
          .then(actual => expect(actual).toEqual([['OK', true], ['OK', false]]))
      );
  });
  ...
  expectPromiseEq('AT+CIPSTATUS', AtInternet.execute(inetState, 'AT+CIPSTATUS'), 'STATUS:2');

  expectPromise('AT+PING', AtInternet.execute(inetState, 'AT+PING="google.com"'), res =>
    expect(res).toMatch(/^\+\d+\nOK/)
  );

  expectPromise('AT+CIPDOMAIN', AtInternet.execute(inetState, 'AT+CIPDOMAIN="xod.io"'), res =>
    expect(res).toMatch(/^\+CIPDOMAIN:[a-z0-9.:]+\nOK/)
  );

  expectPromise(
    'AT+CIPSTART (TCP, single, no keepAlive)',
    AtInternet.execute(inetState, 'AT+CIPSTART="TCP","35.184.230.84",80'),
    res => expect([res, AtInternet.hasConnections(inetState)]).toEqual(['OK', true])
  );
  expectPromise(
  ...
  test('AT+CIPCLOSE of one connection', () => {
    const state = AtInternet.getDefaultState();
    return AtInternet.execute(state, 'AT+CIPSTART="TCP","35.184.230.84",80').then(r0 =>
      AtInternet.execute(state, 'AT+CIPCLOSE=0').then(r1 => expect([r0, r1]).toEqual(['OK', 'OK']))
    );
  });
  ...
  test('AT+CIPCLOSE of all connection', () => {
    const state = AtInternet.getDefaultState();
    return AtInternet.execute(state, 'AT+CIPMUX=1')
      .then(() => AtInternet.execute(state, 'AT+CIPSTART=0,"TCP","35.184.230.84",80'))
      .then(() => AtInternet.execute(state, 'AT+CIPSTART=1,"TCP","35.184.230.84",80'))
  ...
  test('TCP: Connect and send', () => {
    ...
    return AtInternet.execute(state, 'AT+CIPSTART="TCP","35.184.230.84",80')
      .then(r0 => AtInternet.execute(state, 'AT+CIPSEND=' + length).then(r1 => [r0, r1]))
      .then(([r0, r1]) =>
        AtInternet.send(state, request).then(r2 =>
          expect([r0, r1, r2]).toEqual(['OK', 'OK\n>', 'Recv 44 bytes\n\nSEND OK'])
        )
      );
  });
    ...
    return new Promise((resolve, reject) => {
      AtInternet.create(answer => {
        if (answer === 'OK') {
          ...
          reject(new Error(`Expected to get \`OK\` answer, but got \`${answer}\`.`));
        }
      })('AT');
    });
  });
