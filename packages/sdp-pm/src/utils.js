...
export const prependVIfNeeded = R.when(
  R.allPass([
    notNil,
    notEmpty,
    R.compose(R.complement(R.equals('v')), R.nth(0)),
...
export const parseLibQuery = R.compose(
  ...
  R.ifElse(R.compose(R.gt(R.__, 1), R.length), Maybe.of, Maybe.Nothing),
  R.reject(R.isEmpty),
  R.tail,
  R.match(/^([^@/]+?)\/([^@/]+?)(?:@([^@/]+?))?$/)
);
...
export const rejectUnexistingVersion = R.curry((params, libdata) =>
  R.ifElse(
    R.propEq('version', LATEST),
    () => Promise.resolve(libdata),
    () =>
      R.ifElse(
        R.compose(R.contains(params.version), R.prop('versions')),
...
export const getSwaggerClient = (() => {
  ...
  return url => {
    ...
    const client = swaggerClient(url).catch(() => {
      delete memo[index];
...
export const swaggerError = err => {
  ...
  if (status === 400) {
    return new Error(
      R.pathOr('Bad request', ['errors', 0, 'errors', 0, 'message'], res)
    );
  }
